# offline_tuner.py
# ============================================================
# 🧪 MLB HR Offline Tuner (No-TODAY, No-Weather) — Cloud Speed
...
# - Optional day-wise LambdaRank (if dates exist) — included, cloud-lean
...
# Base models (lean but accurate with early stop)
...
# ---------- Defaults for tuners ----------
DEFAULT_BLEND = dict(
    w_prob=0.30,     # base prob
    w_overlay=0.20,  # overlay
    w_ranker=0.20,   # LambdaRank
...
# ---------- Robust merge like the prediction app ----------
def _to_key_str(s):
    return s.astype(str).str.strip().str.lower()

with st.spinner("Merging profiles into event-level…"):
    ev = ev.copy()
    # make key columns for merge (string-safe)
    ev["bat_key_merge"] = _to_key_str(ev[ev_batter_key].astype(str))
    ev["pit_key_merge"] = _to_key_str(ev[ev_pitcher_key].astype(str))
    ...
    if f"batprof_{bat_key}" not in bat_pref.columns:
        ...
        st.stop()
    bat_pref["bat_key_merge"] = _to_key_str(bat_pref[f"batprof_{bat_key}"].astype(str))
    ...
    if f"pitprof_{pit_key}" not in pit_pref.columns:
        ...
        st.stop()
    pit_pref["pit_key_merge"] = _to_key_str(pit_pref[f"pitprof_{pit_key}"].astype(str))
...
# Avoid obvious leakage — same list you’ve used
LEAK = {
...
# Column knobs (adjust if your profiles differ)
bat_cols = dict(
    barrel="batprof_barrel_rate",     # e.g. 0.11
    fb="batprof_fb_rate",             # e.g. 0.22
    pull="batprof_pull_rate",         # e.g. 0.35
    hr_pa="batprof_hr_per_pa"         # optional
)
pit_cols = dict(
...
# Cloud-lean random search count
...
# Final overlay using chosen exponents
...
export_payload = {
    ...
    "notes": "Weather-free overlay exponents + final blend weights from offline tuner (cloud-lean)."
}
st.download_button(
